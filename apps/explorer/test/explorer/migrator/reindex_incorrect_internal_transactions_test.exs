defmodule Explorer.Migrator.ReindexIncorrectInternalTransactionsTest do
  use Explorer.DataCase, async: false

  alias Explorer.Chain.PendingBlockOperation
  alias Explorer.Migrator.{ReindexIncorrectInternalTransactions, MigrationStatus}
  alias Explorer.Repo
  ...
  describe "Migrate incorrect internal transactions" do
    test "Adds new pbo for incorrect internal transactions" do
      ...
      assert MigrationStatus.get_status("reindex_incorrect_internal_transactions") == nil
      assert Repo.all(PendingBlockOperation) == []

      ReindexIncorrectInternalTransactions.start_link([])
      Process.sleep(100)
      ...
      assert MigrationStatus.get_status("reindex_incorrect_internal_transactions") == "completed"
    end
  end
end
